...
```{r}
url <-
  "https://dataverse.harvard.edu/api/access/datafile/3086908?gbrecs=true"
  directory <- proc_dir
  file <-
  file.path(
  directory,
...
d_nei_unique <- unique(d_nei[, .(
  ...
  d_nei_unique <-
  d_nei_unique[Facility.ID..ORISPL. != "" & Unit.ID != ""]
  d_nei_unique <-
  d_nei_unique[, Facility.ID..ORISPL. := as.numeric(d_nei_unique$Facility.ID..ORISPL.)]
  head(d_nei_unique)
```
...
```{r}
d_ampd_annual <-
  unique(d_ampd_annual, by = c("Facility.ID..ORISPL.", "Unit.ID"))
  d_ampd_annual <-
  d_ampd_annual[, Facility.ID..ORISPL. := as.numeric(d_ampd_annual$Facility.ID..ORISPL.)]
  head(d_ampd_annual)
```
...
M <-
  ...
  file <-
  file.path(directory, 'merge_no_missing_info_coal_only_2005.csv')
  fwrite(M, file)
```
