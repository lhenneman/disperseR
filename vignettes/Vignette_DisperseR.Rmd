...
```{r}
hysp_raw <- run_disperser_parallel(input.refs = input_refs_subset,
...
```{r, fig.height = 7, fig.width = 7}
zip_exp_ann_plot <- plot_impact_weighted(data.linked = zip_exp_ann,
  data.units = unitsrun,
  zcta.dataset = zcta_dataset,  
  time.agg = 'year',
  metric = 'hyads',
...
zip_exp_unit <- plot_impact_unit(data.linked = zip_exp_unit_mon, 
  zip.codes = zipcodes,
  graph.dir = graph_dir)
```
