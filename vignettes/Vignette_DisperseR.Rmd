...
If you, for example, you want to run `link_all_units_zip()` for Jan-March 2005, you just have to use the `get_yearmon()` function and set  `start.year = "2005"`, `start.month = "01"`, `end.year = "2005"`, and `end.month = "03"`.
...
```{r}
data_linked_zips <- disperseR::link_all_units_zip(units.run=unitsrun, 
  mc.cores = parallel::detectCores(), 
  year.mons = yearmons,
  pbl.height = pblheight,
  crosswalk. = crosswalk,
...
```{r}
impact_table_single <- disperseR::create_impact_table_single(data.linked=data_linked_zips, 
  data.units = unitsrun,
...
ziplink_plot <- disperseR::plot_impact_single(data.linked = data_linked_zips, 
  map.unitID = "3136-1",
  map.month = "20061", 
  data.units = unitsrun,
  zcta.dataset = zcta_dataset,  
...
```{r}
combined_ziplinks <- disperseR::combine_monthly_ziplinks(month_YYYYMMs = yearmons)
