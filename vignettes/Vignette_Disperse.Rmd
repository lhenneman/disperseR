...
Equally important is the ZCTA shape file. Using the `get_data()` function and setting `data = "zctashapefile"` you can download and preprocess the file from the [US census website](http://www2.census.gov/geo/tiger/GENZ2017/shp/cb_2017_us_zcta510_500k.zip) automatically. If the file already exists in the correct folder, this function will preprocess it and load it into your R environment. 
...
Here we also illustrate how to use the limit arguments to zoom in on the map. By setting `x.lim = c(-66, -84)`, for example, we restrict the map only to the map to display only points within the 65$^{\circ}$W and 85$^{\circ}$W longitude points. 
...
```{r, fig.height = 4, fig.width = 7}
zipcodes <- c(16226, 10003,  10001, 26720)
