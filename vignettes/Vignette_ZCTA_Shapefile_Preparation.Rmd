...
In this vignette, we show how we prepared the ZCTA shape that can be downloaded using the `get_data()` function specifying the `data` argument to be `"zctashapefile"`. In case you want to use different files you can follow some of these preprocessing steps. 
...
```{r}
zcta <- file.path(zcta_dir, 'cb_2017_us_zcta510_500k.shp')
