  ...
  out <- tryCatch(
    {
      ...
      if (substr(file, nchar(file)-3+1, nchar(file))=="zip"){ # if a zip file unzip it.
        unzip(file, exdir = dir)
      }
    },
    error=function(cond) {
      message(paste("Sometimes this URL does not connect properly. In this case, you can try again later"))
...
get_data<-function(data, start.year=NULL, start.month=NULL, end.year=NULL, end.month=NULL, unit=NULL, linked_zips_data=NULL, linked_zips_data_position=NULL) {
  ...
  if(data=="pblheight"){ ## planetary layers data
    directory <- hpbl_dir
  ...
  if(data=="metfiles"){
    if (is.null(startyear) | is.null(startmonth) | is.null(endyear) | is.null(endmonth)) {
      stop("Please specify the metfiles dates correctly. Please refer to the documentation")
    }
    inputdates <-  c(paste(startyear, startmonth, "01", sep="/"), paste(endyear, endmonth, "01", sep="/"))
  ...
  if(data!="metfiles"){
    if(!file.exists(file)){
      message("This might take a bit, but sometimes the url will fail to connect, in this case you might want to try again later")
  ...
  if(data=="zcta_dataplot"){
    ...
    zcta <- merge(zcta, hyspdisp2::crosswalk, by = "ZCTA", all = F, allow.cartesian = TRUE)
    return(zcta)
  }
  ...
  if(data=="zctashapefile"){
    ...
    message("The zcta shape files are now downloaded and preprocessed sucessfully!")
    return(zcta.trans)
  }
